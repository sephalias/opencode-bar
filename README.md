...
- Usage windows come from the official `https://opencode.ai/zen/go/v1/usage` API using the same API key (no extra setup).
...
OpenCode Go reads the API key from the OpenCode auth entry `opencode-go`. Current usage comes from the official `zen/go/v1/usage` API with that key.
